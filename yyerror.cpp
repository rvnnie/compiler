...
TRUE FALSE IF ELSE STATIC INT BOOL CHAR WHILE FOR RETURN BREAK IN
SUB MUL QUESTION DOESNOT ADD DIV SEMICOLON COMMA COLON
EQUAL OR AND LESS GREAT MOD CHSIGN
LEFTPARAN RIGHTPARAN LEFTBRACK RIGHTBRACK LEFTSQBRACK RIGHTSQBRACK
ADDASS SUBASS MULASS DIVASS EQ GEQ LEQ NEQ DEC INC
*/
void initErrorProcessing() {
    ...
    niceTokenNameMap["RIGHTPARAN"] = (char *)"\')\'";
    niceTokenNameMap["LEFTPARAN"] = (char *)"\'(\'";
    niceTokenNameMap["RIGHTBRACK"] = (char *)"\']\'";
    niceTokenNameMap["LEFTBRACK"] = (char *)"\'[\'";
    niceTokenNameMap["RIGHTSQBRACK"] = (char *)"\'}\'";
    niceTokenNameMap["LEFTSQBRACK"] = (char *)"\'{\'";
...
void yyerror(const char *msg)
{
    ...
    // print components
    printf("ERROR(%d): Syntax error, unexpected", line);
    if(strcmp(strs[3], "\"true\"") == 0 || strcmp(strs[3], "\"false\"") == 0) printf(" Boolean constant");
    printf(" %s", strs[3]);
    if (elaborate(strs[3])) {
        if (yytext[0]=='\'' || yytext[0]=='"') printf(" %s", yytext); 
        else {
            if(strcmp("forever", yytext) == 0) printf(" \"for\"");
